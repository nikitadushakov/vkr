...
class generatorClass: # simulates generator behaviour, unused
	...
	def __init__(self, modelClass, height = 480, width = 640, batchSize = 2, outVectors = False, outClasses = False, outVecName = None, outClassName = None):
		...
		self.i = 0
		self.basePath = os.path.dirname(os.path.join(os.path.realpath(__file__)), 'LINEMOD',  modelClass)
		self.masterList = data.getMasterList(self.basePath)
	...
	def serveBatch(self):
		...
		for b in range(self.batchSize):
			if self.i == self.dataLength:
				...
				return True, [], [], []
			x = data.filePathToArray(os.path.join(self.basePath, 'JPEGImages', self.masterList[self.i][0]), self.height, self.width)
			
			with open(os.path.join(self.basePath, 'labels' , self.masterList[self.i][2])) as f:
			...
			modelMask = data.filePathToArray(os.path.join(self.basePath ,'mask', self.masterList[self.i][1]), self.height, self.width)
			modelCoords = np.where(modelMask == 255)[:2]
			
			for modelCoord in zip(modelCoords[0][::3], modelCoords[1][::3]):
				data.setTrainingPixel(yCoordsLabels, modelCoord[0], modelCoord[1], labels, self.height, self.width)
